...
const reaction = ({msg, data, reaction, chatuserinfo, setUpdatemsgs, socket}) => {
  ...
  return (
    <div>
      {msg.text ?
        <div
          className="absolute cursor-pointer z-40 -bottom-8 border border-slate-800 bg-slate-900 text-slate-100 py-1 px-1 rounded-full text-xs shadow-md hover:scale-110 transition-transform"
          onClick={() => {
        ...
        : <div
          className="absolute cursor-pointer z-40 -bottom-7 border border-slate-800 bg-slate-900 text-slate-100 p-0.5 px-1 rounded-full text-xs shadow-md hover:scale-110 transition-transform"
          onClick={() => {
            if (reaction.user._id === data._id) {
              undoReaction(msg._id, reaction._id, chatuserinfo.userId);
            }
