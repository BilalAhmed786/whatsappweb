...
const Input = ({
  ...
  const handleSend = async (e) => {
    ...
    const blockeduser = data.blockedUsers.some(
      (user) => user.userId === chatuserinfo.userId,
    );
    ...
    if (
      ...
      if (replymessage.objectId) {
        formData.append(
          "objectid",
          replymessage.objectId && replymessage.objectId,
        );
      }

      try {
        const response = await axios.post(
          `${backendbaseurl}/api/files/uploads`,
          formData,
          { withCredentials: true },
        );

        if (response.data) {
          socket?.emit("latestmsg", {
            lastmsg: response.data,
            receiverid: chatuserinfo.userId,
          });
    ...
    <div className="bg-slate-950/80 border-t border-slate-800/80">
      <div className="w-full">
        <div className="relative w-full flex justify-between items-center">
          ...
          <div className="flex items-center w-11/12 bg-slate-900 border border-slate-800 rounded-lg shadow-md">
            <ReplyMessage
