...
const Home = () => {
  ...
  const { socket, data, chatuserinfo, setChatuserinfo, fetchUserInfo } =
    ...
    const notificationdata = async () => {
      try {
        const res = await axios.get(
          `${backendbaseurl}/api/notification/allnotification`,
  ...
  useEffect(() => {
    ...
    const handlechatuser = (data) => {
      setChater((prev) => {
        const userexist = prev.some(
          (user) => user.loginuser === data.loginuser,
        );
        ...
        if (userexist) {
          ...
          return [
            ...prev,
            { loginuser: data.loginuser, chatuser: data.chatuser },
          ];
        }
      });
    };
  ...
  return (
    <div className="flex w-full h-full overflow-hidden">
      {myprofile ? (
        <div
          className={`lg:flex-[1] lg:relative lg:translate-x-0 w-full bg-white z-20 transition-transform duration-300 ease-in-out
